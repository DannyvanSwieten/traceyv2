...
pub fn start_render_loop(
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            // Recompile if needed, then render
            if let Ok(mut engine) = engine.lock() {
                if needs_recompile {
                ...
                // Check if we have a native viewport to present to
                let has_native_viewport = viewport_manager
                    .lock()
                    .map(|vm| vm.has_native_viewport())
                    .unwrap_or(false);

                // Render frame (skip pixel readback if presenting to native viewport)
                let need_pixels = !has_native_viewport;
                match engine.render_frame(&current_camera, clear_accumulation, need_pixels) {
                    Ok(result) => {
                        ...
                        // Present to native viewport if available
