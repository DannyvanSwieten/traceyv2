...
// Mirrors src/sops/serialization.cpp. Same schema, same parameter encoding,
// same connection encoding — only the registry queried at deserialize time
// and the `graph_kind` literal differ. Keep the two files in sync.
...
namespace tracey
{
    namespace vops
    {
        namespace
        {
            ...
            void applyParamFromJson(VopNode &node, const std::string &name, const json &j)
            {
            ...
            json graphToJson(const VopGraph &graph)
            {
                json root;
                root["graph_kind"] = "vop";
                root["version"] = 1;
                ...
                {
                    if (auto *vn = dynamic_cast<const VopNode *>(n.get()))
                    {
                        nodes.push_back(nodeToJson(*vn));
                    }
                }
            ...
            std::string serialize_internal(const VopGraph &graph, bool pretty)
            {
        ...
        std::unique_ptr<VopGraph> deserializeVopGraph(const std::string &jsonText)
        {
            ...
            if (root.value("graph_kind", std::string{"vop"}) != "vop")
                throw std::runtime_error("deserializeVopGraph: graph_kind != 'vop'");
            const int version = root.value("version", 1);
            if (version != 1)
                throw std::runtime_error("deserializeVopGraph: unsupported version "
                    + std::to_string(version));

            auto graph = std::make_unique<VopGraph>(root.value<size_t>("uid", 0));
            ...
            if (root.contains("nodes") && root["nodes"].is_array())
            {
                for (const auto &nj : root["nodes"])
                {
                    ...
                    auto node = VopRegistry::instance().create(kind, uid);
                    if (!node)
                        throw std::runtime_error(
                            "deserializeVopGraph: unknown node kind '" + kind + "'");
